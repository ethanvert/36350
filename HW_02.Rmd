...
while (homogeneous == FALSE) {
  counter <- counter + 1
  outcome <- rbinom(1, 8, 0.5)
  if (outcome == 0 || outcome == 8) {
    break
  }
}
print(counter)
...
```{r linewidth=80}
char2int = function(letter)
{
  if (typeof(letter) != "character" || length(letter) > 1 || nchar(letter) > 1 || !suppressWarnings(is.na(as.integer(letter))))
  {
    return (NULL)
  } else {
    for ( i in 1:26 ) {
...
my_factorial = function(num) {
  if (typeof(num) != "double" || num %% 1 != 0 || num < 0) {
    return (NULL)
  } else if (num == 0) {
    return (1)
  }
...
root = function(x, t=10^(-6)) {
  ...
  return (list(sqrt.x = r, n.iter = depth))
}
root(11)[["sqrt.x"]]
