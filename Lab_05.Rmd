...
```{r linewidth=80}
simple <- read.table("simple.txt", header = TRUE)
names(simple)
```
...
```{r linewidth=80}
hist(simple$redshift, main = "Histogram of Planets' Redshift", xlab = "Redshift", col = "pink")
```
...
```{r linewidth=80}
read.table("simple.txt", header = TRUE, nrows = 4)
```
...
```{r linewidth=80}
stu <- read.table("students.txt", header = TRUE)
typeof(stu$Last.Name)
```
...
```{r linewidth=80}
plot(emline$mass, emline$H_ALPHA, main = "H_ALPHA vs. mass", 
     xlab = "mass",
     ylab = "H_ALPHA",
     type = "p",
     col = "orange")
...
```{r linewidth=80}
"emline.csv" %>% read_csv(.) %>% suppressMessages(.) %>% complete.cases(.) %>% sum(.)
```
...
```{r linewidth=80}
suppressMessages(read_csv("students.csv", skip = 6, col_types = "cccf"))
```
...
#key = PUT YOUR KEY HERE and UNCOMMENT LINE
