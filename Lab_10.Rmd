...
v[1] <- as.character(as.Date(date[1], "%B %d, %Y"))
v[2] <- as.character(as.Date(date[2], "%d %B %Y"))
v[3] <- as.character(as.Date(date[3], "%m-%d-%Y"))
v[4] <- as.character(as.Date(date[4], "%m/%d/%Y"))
v[5] <- as.character(as.Date(sub("(..)$", "19\\1", date[5]), "%m.%d.%Y"))
(Date <- as.Date(v, "%Y-%m-%d"))
```
...
```{r linewidth=80}
set.seed(101)
num.days <- julian(as.Date("1/1/2000", "%m/%d/%Y"), as.Date("1/1/1990", "%m/%d/%Y"))[1]

days <- as.Date("1/1/1990", "%m/%d/%Y") + sample(0:num.days, 5)
days[order(days)]
```
...
```{r linewidth=80}
dt <- vector("character", 5)
dt[1] <- as.character(as.POSIXlt(dts[1],format="%B %d, %Y %I:%M %p"))
dt[2] <- as.character(as.POSIXlt(dts[2],format="%d %B %Y %H:%M:%S"))
dt[3] <- as.character(as.POSIXlt(dts[3],format="%m-%d-%Y %I:%M:%S %p"))
dt[4] <- as.character(as.POSIXlt(dts[4],format="%m/%d/%Y %I:%M"))
dt[5] <- as.character(as.POSIXlt(paste(sub("(..)$", "19\\1", date[5]), time[5]), 
                                 format="%m.%d.%Y %I:%M %p"))
...
```{r linewidth=80}
diff <- as.numeric(dt[2]-dt[3]) * 24 * 60
cat("Time difference of", diff," minutes")
```
