...
bdays <- regmatches(sprint.lines, regexpr("[0-9]{2}\\.[0-9]{2}\\.[0-9]{2}",sprint.lines,useBytes=TRUE))
bdays <- as.Date(sub("(..)$", "19\\1", bdays), "%d.%m.%Y")

sprint.dates <- regmatches(sprint.lines, regexpr("[0-9]{2}\\.[0-9]{2}\\.[0-9]{4}",sprint.lines,useBytes=TRUE))
sprint.dates <- as.Date(sprint.dates, "%d.%m.%Y")
diff <- sprint.dates - bdays


hist(as.numeric(diff), main = "Distribution of Sprinter Age",
                       xlab = "Sprinter Age (Days)",
                       ylab = "Density")
```
