...
```
```
The shorter vector is iterated upon until the longer vector has been fully multiplied.
```
...
```{r linewidth=80}
m <- matrix(1:20 * c(0,0,0,1), nrow = 4, ncol = 5)
m
```
...
```{r linewidth=80}
sort(setdiff(union(u,v), intersect(u,v)))
sum(is.element(c(u,v), setdiff(union(u,v), intersect(u,v))))
```
...
sum(is.element(setdiff(v,u)[setdiff(v,u) > 50],setdiff(v,u)))
```
